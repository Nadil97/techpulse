const PROVIDER = (import.meta.env.VITE_PROVIDER || "openrouter");
function cleanKey(v) {
...
export async function chatDirect(messages) {
    ...
    if (PROVIDER === "openrouter") {
        ...
        if (!key.startsWith("or_"))
            console.warn("OpenRouter key doesn’t look right.");
    ...
    if (PROVIDER === "hf") {
        const raw = import.meta.env.VITE_HF_API_KEY;
        const key = cleanKey(raw);
    ...
    const txt = await resp.text();
    if (!resp.ok)
        throw new Error(extractErr(txt) || `OpenAI ${resp.status}`);
    const data = JSON.parse(txt);
    return data?.choices?.[0]?.message?.content?.trim() || "No reply.";
}
function toHFPrompt(messages) {
    let out = "";
