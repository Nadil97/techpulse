...
export function buildStoryContext(item, related = []) {
    const head = `Story:
- Title: ${item.title}
- Source: ${item.source}
- URL: ${item.url ?? ""}
...
export function buildCompareContext(items) {
    return [
        "Compare these stories:",
        ...items.slice(0, 4).map((it, i) => `(${i + 1}) ${it.title} — ${it.source} — ${it.url ?? ""}`),
    ].join("\n");
}
export function summarizeInstruction(url) {
    return [
